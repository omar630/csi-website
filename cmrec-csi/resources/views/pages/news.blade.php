@extends('layouts.app')

    <section id="logo">
             <h1 style="color:white;text-align:center;padding-top:10px;">CMREC-CSI</h1>    
       </section>
          @include('navbar')
<section class="text-center;">
<div class="container">
        <div class="row">
            <div class="column">&nbsp;</div>
        </div>
    </div>
    <div class="container-fluid">
        <div class="row">
            
            <div class="col-md-12">
                <div class="list-group">
  
 <div class="container" align=center>
 <table style="border:1px solid #CCC;width:80%;margin-top:20px;" cellpadding=0 cellspacing=0 class="table responsive table-hover">
        <tr style="background:#002060 !important;"><td colspan=3 class="text-center">
        <a href="######" class="list-group-item list-group-item-action active" style="background:#002060 !important;border:#dc143c !important">
   <h4 style="font-size:20px;"> LATEST NEWS</h4>
  </a></td>
        </tr>
        <tr>
            <th class="text-center">S. No.</th>
            <th class="text-center">News Headline</th>
            <th class="text-center">News Date</th>
        </tr>

<tr>
    <td width="10%">1</td>
    <td width="65%"><a target=_blank href="#####">News 1</a></td>
    <td width="25%" style="text-align:right">XX-XXX-XXXX</td>
</tr>   
<tr>
    <td width="10%">2</td>
    <td width="65%"><a target=_blank href="#####">News 2</a></td>
    <td width="25%" style="text-align:right">XX-XXX-XXXX</td>
</tr>   

    </table>
</div>

            </div>
            
        </div>
    </div>




</section>